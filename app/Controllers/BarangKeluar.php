...
class BarangKeluar extends BaseController
{
    ...
    public function index()
    {
        ...
        $data = [
            'title' => 'Inventory Kantin | Barang Keluar',
    ...
    public function get_datatable()
    {
        ...
        $data = $this->Mdetail->get_data_barang_keluar($start);
        // dd($data);

        foreach ($data as &$row) {
            $row['action'] = '
                <button class="btn btn-sm btn-primary edit-btn-keluar" data-id="' . $row['iddetail'] . '"><i class="fas fa-pen"></i></button>
                <button class="btn btn-sm btn-danger delete-btn-keluar" data-id="' . $row['iddetail'] . '"><i class="fas fa-trash"></i> </button>
            ';
        }
    ...
    public function create()
    {
        ...
        if ($this->Mdetail->insert($dataDetail)) {
            ...
            if ($currentStock) {
                $currentStock = (int)$currentStock['stok'];
                ...
                if ($newStock >= 0) {
                    ...
                    return $this->response->setJSON(['message' => 'Stok barang tidak cukup'], 400);
                }
            } else {
                $this->db->transRollback();
